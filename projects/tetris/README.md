...
- cell 타입이 원시값이다보니 확장하는데 어려움이 있었음. 그래서 객체 형태로 분리하고 type, shadow, disabled를 객체안에 넣음.
- 이후 여러가지 인터렉션들을 넣기 위한 사전작업임.
- 다행히 100개가 넘는 테스트코드 덕분에 안정적으로 마이그레이션 할 수 있었다.
- 또한 테스트 코드를 수정하기 위해 가벼운 convert 함수를 만들어서 처리했다.
