...
- 값 객체(value object)(VO): 정체성이 없는 데이터
...
## 7. 파이프라인으로 작업흐름 모델링하기

- 상태기계(state machine): 문서나 레코드는 하나 이상의 상태에 있을 수 있으며, 명령에 따라 상태가 전이된다.
- 합타입을 이용해서 처리하면 된다. `type ShoppingCart = | EmptyCart | ActiveCart | PaidCart;`
- 참, 거짓은 정보가 빈약하여 좋은 디자인이 아니다. enum을 사용하면 좋다.
- TaskEither 등을 사용해서 에러가 발생하는 경우 확실하게 정의하기

- 의존을 디자인에 포함시켜야 할까? 항상 옳은 정답이 있지는 않다.

  - 공개 API에 노출되는 함수의 경우, 호출자에게 의존 정보를 숨겨라.
  - 내부적으로 사용되는 함수의 경우, 의존을 명시하라.

- 외부 서비스를 완료하는데 시간이 많이 걸리는 경우가 있다. 각 단계 사이에서 상태를 유지해야 한다. 이러한 실행 작업 흐름을 종종 saga라고 한다.

# Part3. 모델 구현하기

## 8. 함수 이해하기

- 커링이 헷갈리면 책 예제를 쭉 읽어보면 좋을듯.
- 함수 합성을 통해 정보 은닉이 가능하다.(사과 => 바나나, 바나나 => 체리, 사과 => 체리)
- pipe, flow를 이용해서 함수 합성을 사용한다.
- 저수준 연산으로 서비스 함수를 만들고, 서비스 함수를 결합하여 작업 흐름을 만든다. 또한, 작업 흐름을 합성해서 애플리케이션을 만든다.
- 리스트, 성공/실패를 나타내는 Either 타입, 비동기 함수 등을 사용할 때 타입 불일치 문제가 발생한다. 인기있는 해결책 중 하나는 양쪽을 동일한 타입으로 변환하는 것이다.(최소공배수 찾기)

## 9. 구현: 파이프라인 조합하기

이 장은 코드 전체를 읽어보면서 흐름을 따라가야함. 코드가 꽤 어려운편임. 직접 가벼운 애플리케이션을 파이프라인으로 만들지 않으면 내껄로 만들기 쉽지 않을듯

- 함수 파이프라인에서 타입이 다르다면 함수 어뎁터를 생성하자
- 파이프라인 중 아직 구현하고 싶지 않거나 구현 방법을 모를 때에는 `throw "not implemented"`
- 객체지향에서는 의존 주입(DI)이나 IOC 컨테이너를 사용하지만, 함수형에서는 '리더 모나드', '프리 모나드'같은 기법을 사용해야한다. (이 책에서는 간단하게 모든 의존을 최상위 함수에서 준비하여 콜 스택을 타고 필요한 곳까지 전달한다. 이거 코틀린만 있음..)
- 일단.. 전통적인 함수형 프로그래밍에서는 의존을 드러낼 때는 함수 매개변수로, 의존을 숨길 때는 리더 모나드를 쓴다는 것만 알고 넘어가자.
- 의존이 늘어나면 상위 함수 의존이 폭발적으로 증가할 수 있다. 이 매개변수들을 계속 위로 전파할필요는 없다. 최상위 함수 외부에서 의존을 부분 적용하여 그 자식 함수를 내려보내면 된다.
- 의존 함수를 매개변수로 전달하는 방식은 테스트 하기가 매우 쉽다. 모킹 라이브러리에 의지하지 않아도 작동하는 가짜 의존을 쉽게 주입할 수 있다.
