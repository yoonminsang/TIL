 ...
 * @description 기준점을 획득한 다음 해당 기준점을 기준으로 배열을 나눈다.
    ...
    2. 2개의 포인터를 생성. 
      2-1. 좌측 포인터에 해당하는 값이 피봇보다 큰 값을 찾을 때 까지 좌측 포인터를 움직임(즉 최종 index는 피봇보다 커짐)
      2-2. 우측 포인터에 해당하는 값이 피봇보다 작은 값을 찾을 때 까지 우측 포인터를 움직인 다음,(즉 최종 index는 피봇보다 작아짐)
...
function partition(arr, leftIndex, rightIndex) {
  ...
  while (leftIndex <= rightIndex) {
    // 2-1. 좌측 포인터에 해당하는 값이 피봇보다 큰 값을 찾을 때 까지 좌측 포인터를 움직임(즉 최종 index는 피봇보다 커짐)
    while (arr[leftIndex] < pivot) leftIndex += 1;
    // 2-2. 우측 포인터에 해당하는 값이 피봇보다 작은 값을 찾을 때 까지 우측 포인터를 움직인 다음,(즉 최종 index는 피봇보다 작아짐)
    while (arr[rightIndex] > pivot) rightIndex -= 1;
...
// In-Place: 기존 객체를 변경
// Out-Place: 새로운 객체 생성
// 아웃 플레이스. 덜 효율적. 위 코드는 인플레이스라서 효율적임)
