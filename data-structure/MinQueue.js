...
class MinQueue extends Queue {
  ...
  enqueue(value) {
    if (typeof value !== 'number') {
      throw new Error(`enqueue's value must be a number`);
    }
    super.enqueue(value);
    if (this.minQueue.isEmpty() || this.minQueue.peek() >= value) {
      console.log('추가');
      this.minQueue.enqueue(value);
    }
  }

  dequeue() {
    const result = super.dequeue();
    // console.log('dequeue', result, this.minQueue.peek());
    if (result === this.minQueue.peek()) {
      console.log('제거');
  ...
  min() {
    console.log(this.minQueue.toString());
